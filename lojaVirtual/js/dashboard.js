...
function updateStats() {
    ...
    const produtosVendidos = vendas.filter(v => v.status === 'completed').length;
...
function fillSalesTable() {
    ...
    vendas.slice().reverse().forEach(venda => {
        ...
        const statusClass = venda.status === 'completed' ? 'completed' : 
                           venda.status === 'pending' ? 'pending' : 'cancelled';
        const statusText = venda.status === 'completed' ? 'Concluído' : 
                          venda.status === 'pending' ? 'Pendente' : 'Cancelado';
        ...
        tr.innerHTML = `
            ...
            <td>${venda.produto}</td>
            <td>${venda.data}</td>
            <td><strong>R$ ${venda.valor.toFixed(2).replace('.', ',')}</strong></td>
            ...
            <td>
                <div class="action-buttons">
                    ...
                    <button class="btn-action" title="Excluir" onclick="deleteVenda('${venda.id}')">
